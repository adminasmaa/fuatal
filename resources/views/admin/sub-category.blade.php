...
@section('content')
    ...
    @endif
                <div class="card-content" style="padding-bottom: 2rem;">
                    <div class="container-fluid">
        <h1 class="title">Subcategory Form</h1>
...
<br>
      ...
      <button class="button is-small is-danger" type="submit" onclick="deleteRecord({{$item->id}})">
            ...
            <a class="button is-small is-primary" href="{{url('admin/subcategory/'.$item->id.'/edit')}}">
            <span class="icon">
                <svg viewBox="0 0 32 32" fill="none" stroke="currentcolor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
            ...
            <span>Edit</span>
            		...
            		<td>
            			@if(is_null($item->subcat_image))
            			<img width="70px" src="{{URL::asset('uploads/categories/')}}">
            			@else
            			<img width="70px" src="{{URL::asset('uploads/categories/'.$item->subcat_image)}}">
            			@endif
            		</td>
            		<td>{{$item->title}}</td>
