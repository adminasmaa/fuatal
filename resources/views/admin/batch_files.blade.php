                        ...
                        <div class="card card-flush">
                            ...
                            <div class="card-header align-items-center py-5 gap-2 gap-md-5" style="display:block">
                                ...
                                <table id="dataTableBuilder" class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer">
                                    ...
                                    <tbody>
                                        @foreach($batch_files as $item)

                                        <tr>
                                            <td class="pe-0">{{$loop->iteration}}</td>
                                            <td class="pe-0">{{$item->file_name}}</td>
                                            <td><a class="btn btn-info btn-sm" href="{{url('admin/print-text-batch-file-download?file_id='.$item->id)}}"><i class="fa fa-download"></i></a></td>
                                            <!-- -->
                                        </tr>
                                        @endforeach
                                    </tbody>
                                ...
                                <div class="col-md-12 mt-4 custom-footer">
                                    {{ $batch_files->appends(request()->input())->links('pagination::bootstrap-4') }}
                                </div>
                            </div>
                        </div>
                    </div>
...
<!-- {{ $batch_files->onEachSide(5)->links() }} -->
<!-- {{ $batch_files->links() }} -->

<!-- {{ $batch_files->appends($_GET)->links() }} -->
@endsection
...
<script type="text/javascript">
    ...
    document.getElementById('pagination').onchange = function() {
        window.location = "{!! $batch_files->url(1) !!}&limit=" + this.value;
    };
