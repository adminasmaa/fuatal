...
@section('content')
    ...
    @endif
                <div class="card-content" style="padding-bottom: 2rem;">
                    <div class="container-fluid">
        <h5 class="title py-5">Add Sticks Range</h5>
    <form method="POST" enctype="multipart/form-data" action="{{route('admin.sticks.save')}}">
         @csrf
         <div class="row mb-5">
            <div class="col-md-6 fv-row">
                <label class="required fs-5 fw-bold mb-2">From</label>
                <select data-control="select2" data-placeholder="Select from range..." class="form-select" name="from">
                    @for ($i = 1; $i <= 99; $i++)
...
<div class="row mb-5">
        <div class="col-md-6 fv-row">
            ...
            <select data-control="select2" data-placeholder="Select to range..." class="form-select" name="to">
                @for ($i = 1; $i <= 99; $i++)
                <option value="{{ $i }}">{{ $i }}</option>
                @endfor
            </select>
