...
@section('content')
    ...
    <form method="POST" enctype="multipart/form-data" action="{{url('admin/translate/city/save')}}">
         @csrf
...
<input type="hidden" name="trans_id" value="{{$cityObject->trans_id}}">
    ...
    <div class="control">
            ...
            <select class="input" name="lang_name" required="required">
               <option value="" selected="selected">--Select--</option>
               @foreach($langObj as $item)
               @if($item->lang_name!='English')
               <option value="{{$item->lang_name}}">{{$item->lang_name}}</option>
               @endif
               @endforeach
            </select>
    ...
    <div class="control">
            <select class="input" name="country_id" disabled="disabled">
