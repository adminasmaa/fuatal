...
public function createFlashRedirect($class, Request $request, $imageColumn = false, $path = 'index')
    {
        ...
        if($request->get('page_form')=='page_form')
        { 
            $latestObj=Page::orderBy('id', 'desc')->first();
            if(!is_null($latestObj))
            {
            ...
            //$trans_id=$transold+1;
              ...
              Page::where('id', $latestObj->id)->update([
                'trans_status'=>0,
                'trans_id'=>$trans_id,
                'lang_name'=>'English'
        ...
        if($request->get('users_form')=='users_form')
          {   
              //$latestObj=User::orderBy('id', 'desc')->first();

              //dd($latestObj->id,$qnames);
              //User::where('id', $latestObj->id)->update([
              User::insert([
