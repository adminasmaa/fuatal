...
abstract class DataTableController extends DataTable
{
    ...
    protected $parameters = [
        ...
        'columnDefs' => [['defaultContent' => 'N/A', 'targets' => '_all']],
        'scrollX' => true,
     ...
     * @return array
     */
    protected function pushColumns(&$result, $data, $order = false, $search = true): array
    {
    ...
    protected function pushOps($datatables, $model = '')
    {
        ...
        if ($this->ops === true) {
            if (empty($model)) {
                ...
                if($model == 'user')
                {
                    $datatables = $datatables->editColumn('is_active', function ($data) use ($model) {
                        if($data->is_active)
                        {
                            return 'Active';
                        } else {
                    ...
                    $datatables = $datatables->editColumn('email', function ($data) use ($model) {
                        if(empty($data->email))
                        {
                            return '-';
                        } else {
                    ...
                    $datatables = $datatables->editColumn('phone', function ($data) use ($model) {
                        if(empty($data->phone))
                        {
                            return '-';
                        } else {
                    ...
                    $datatables = $datatables->editColumn('name', function ($data) use ($model) {
                        if(empty($data->name))
                        {
                            return '-';
                        } else {
                    ...
                    $datatables = $datatables->editColumn('created_at', function ($data) use ($model) {
                    ...
                    $datatables = $datatables->editColumn('updated_at', function ($data) use ($model) {
                ...
                $datatables = $datatables->addColumn('ops', function ($data) use ($model) {     
                    return view('partials.admin.ops', ['resource' => $model, 'id' => $data->id, 'data' => $data]);
                });
            }
            $this->raw_columns[] = 'ops';
            //$this->raw_columns[] = 'Image';
        }
